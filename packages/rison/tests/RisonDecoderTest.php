...
use PiedWeb\Rison\RisonDecoder;

class RisonDecoderTest extends \PHPUnit\Framework\TestCase
{
    public function testArraysOOP(): void
    {
        $r = new RisonDecoder('!(foo,bar)');
        $this->assertEquals(['foo', 'bar'], $r->decode());
    }

    public function testObjectsOOP(): void
    {
        $r = new RisonDecoder('(foo:bar)');
        $this->assertEquals(['foo' => 'bar'], $r->decode());
    }
